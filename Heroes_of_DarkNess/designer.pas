...
begin
  ...
  for q := 1 to MaxLev do begin
    ...
    MessageDlg('Зачем столько чуваков?'+IntToStr(q)+' ?',mtWarning,[mbOk],0);
    GridError := True;
    el := q;
    end;
    if BlipStatus=stNotFound then begin
    MessageDlg('Может нужно поставить героя '
    +IntToStr(q)+' ?',mtWarning,[mbOk],0);
    GridError := True;
    el := q;
    end;
    if HeartsStatus=stNotFound then begin
    MessageDlg('Где жизни '
    +IntToStr(q)+' !',mtWarning,[mbOk],0);
    GridError := True;
    el := q;
    end;
    if ExitStatus=stNotFound then begin
    MessageDlg('А где же выход '
    +IntToStr(q)+' ?',mtWarning,[mbOk],0);
    GridError := True;
    el := q;
    end;
...
begin
  ...
  if OpenDialog1.Execute then begin
      ...
      DesignerForm.Caption := 'Редактор уровней - '+ExtractFileName(DatFileName);
      FileLabel.Caption:= 'Файл: '+ExtractFileName(DatFileName);
...
begin
  ...
  else MessageDlg('Файл содержит до 15 уровней.',mtWarning,[mbOk],0);
end;
...
begin
  ...
  MessageDlg('Энергия может быть о 20 до 200 у.е.',mtWarning,[mbOk],0);
  edEnergy.Value := 100;
